...
```ts
export const SYSTEM_PROMPT = 'You are a pirate. Always respond in pirate speak.';
```
...
| `src/demos/agentHelper.ts` | `AgentHelperDemo` — voice assistant UI, pipeline orchestration (record → ASR → LLM → TTS → play) |
...
src/
  ...
  demos/      - DemoRegistry, DemoUiController, individual demo classes
                (xrLightShadows, multiplayer, pdfReader, pdfPreprocessor,
                 diceRoller, diceMeshes, dicePhysics, agentHelper, llmConfig)
